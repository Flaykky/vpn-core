...
void openvpn_cleanup(OpenVPNContext *ctx) {
    ...
    if (ctx->config) {
        secure_clear_memory(ctx->config->password, strlen(ctx->config->password));
        free(ctx->config->server);
        free(ctx->config->username);
        free(ctx->config->password);
        free(ctx->config->ca_cert);
        free(ctx->config->client_cert);
        free(ctx->config->client_key);
        free(ctx->config);
    }

    pthread_mutex_unlock(&ctx->mutex);
    log_info("OpenVPN resources cleaned up");
}