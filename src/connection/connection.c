...
bool initialize_connection(ConnectionState *state, const char *server_ip, int port) {
    if (!state || !server_ip || port <= 0) return false;
    
    int sockfd = establish_tcp_connection(server_ip, port);
...
SSL* create_ssl_connection(int sockfd, const char *hostname) {
    ...
    // Устанавливаем SNI (Server Name Indication), если требуется
    if (!SSL_set_tlsext_host_name(ssl, hostname)) {
        log_error("Failed to set TLS SNI");
        SSL_free(ssl);
        return NULL;
    }
    ...
    if (SSL_connect(ssl) <= 0) {
        log_error("SSL_connect failed: %s", ERR_error_string(ERR_get_error(), NULL));
        SSL_free(ssl);
        return NULL;
    }
...
bool reconnect(ConnectionState *state) {
    ...
    close_ssl_connection(state->ssl);
    close_connection(state->socket_fd);

    // Новая попытка
    int sockfd = establish_tcp_connection(state->server_ip, state->port);
    if (sockfd < 0) {
        pthread_mutex_unlock(&connection_mutex);
        return false;
    }

    SSL *ssl = create_ssl_connection(sockfd, state->server_ip);
    if (!ssl) {
        close(sockfd);
        pthread_mutex_unlock(&connection_mutex);
        return false;
